...
function isCustomerSetupPath(pathname: string) {
  return pathname.startsWith('/login') || pathname.startsWith('/signin') || pathname.startsWith('/signup') || pathname.startsWith('/onboarding');
}
...
export default function AccountControl() {
  ...
  if (isCustomerSetupPath(pathname) || !isLoaded || !isSignedIn) return null;

  const email = user?.primaryEmailAddress?.emailAddress || '';
  const label = user?.firstName || email.split('@')[0] || 'Account';
