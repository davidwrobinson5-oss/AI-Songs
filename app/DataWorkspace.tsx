...
type DataRequest={id:string;genre:string;geography:string;categories:string[];notes?:string;status:string;requested_count?:number;delivered_count?:number;provider_status?:Record<string,string>;provider_summary?:string;created_at:string;updated_at:string;fulfilled_at?:string;cancelled_at?:string};
...
export default function DataWorkspace(){
  ...
  return <main className="growthWorkspace">
    <section className="hero"><p className="eyebrow">Find the Right People</p><h1>Data</h1><p className="sub">Build targeted audience and industry lists for the exact stage of your music business—fans, media, radio, booking, labels, film/TV, events, sponsors, and the people who can move the next step forward.</p></section>
    <section className="panel"><div className="controlGrid"><div className="statusBox"><small>PIE STAGE</small><strong>{level}</strong></div><div className="statusBox"><small>MONTHLY DATA CREDITS</small><strong>{existingBeta?'Beta':credits.toLocaleString()}</strong></div><div className="statusBox"><small>SAVED REQUESTS</small><strong>{stats.total}</strong></div><div className="statusBox"><small>DELIVERED RECORDS</small><strong>{stats.delivered.toLocaleString()}</strong></div></div><p style={{color:'#8e8f9f',fontSize:11,lineHeight:1.5,marginBottom:0}}>Pie stores requests privately and should only deliver licensed, permissioned, public-business, or otherwise lawfully sourced records with provenance, refresh dates, suppression handling, and provider terms respected. Sensitive personal data is excluded.</p></section>
    ...
    <section className="panel"><div className="songsSectionHead"><strong>Request History</strong><span>{requests.length}</span></div><div style={{display:'grid',gap:8,marginTop:10}}>{requests.length===0?<div className="statusBox">No backend requests yet.</div>:requests.map(r=><article key={r.id} className="statusBox" style={{display:'grid',gap:5}}><div style={{display:'flex',justifyContent:'space-between',gap:12}}><strong>{r.categories.join(' · ')}</strong><small>{r.status}</small></div><small>{r.genre} · {r.geography} · {new Date(r.created_at).toLocaleString()}</small>{r.notes&&<small>{r.notes}</small>}<small>Delivered: {Number(r.delivered_count||0).toLocaleString()}{r.provider_summary?` · ${r.provider_summary}`:''}</small>{!['fulfilled','cancelled'].includes(r.status)&&<button type="button" className="secondary" onClick={()=>cancel(r.id)} disabled={busy}>Cancel Request</button>}</article>)}</div></section>
