...
export default function MapboxOpportunityMap({query,location,token}:{query:string;location:string;token:string}){
  ...
  const search=useCallback(async(useCurrentArea:boolean)=>{
    ...
    try{
      ...
      markerRefs.current.forEach(marker=>marker.remove());markerRefs.current=[];if(window.mapboxgl){markerRefs.current=next.map(place=>new window.mapboxgl!.Marker({color:'#ff3d81'}).setLngLat(place.coordinates).setPopup(new window.mapboxgl!.Popup({offset:18}).setText(`${place.name}${place.address?` — ${place.address}`:''}`)).addTo(map))}
