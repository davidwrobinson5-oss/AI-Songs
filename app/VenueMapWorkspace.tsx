...
export default function VenueMapWorkspace({onNavigate}:{onNavigate:(screen:string)=>void}){
  ...
  return <div style={{display:'grid',gap:16}}>
    ...
    <section className="panel" style={{padding:10,overflow:'hidden'}}>{mapboxToken?<MapboxOpportunityMap query={query} token={mapboxToken}/>:<><iframe title="Google Maps live music opportunity search" src={mapUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen style={{display:'block',width:'100%',height:'52vh',minHeight:390,border:0,borderRadius:16}}/><div className="statusBox" style={{marginTop:10}}>Mapbox is ready in Pie. Add the Mapbox public access token in Vercel to activate Search This Area and automatic map searching.</div></>}<div className="mixButtons"><a className="primary" href={mapsSearchUrl} target="_blank" rel="noreferrer" style={{display:'block',textAlign:'center',textDecoration:'none'}}>Open Full Google Maps</a><a className="secondary" href={calendarSearchUrl} target="_blank" rel="noreferrer" style={{display:'block',textAlign:'center',textDecoration:'none'}}>Verify Booking + Opportunities</a></div></section>
