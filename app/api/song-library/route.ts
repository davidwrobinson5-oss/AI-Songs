...
type CloudVersion = {
  ...
  files?: Record<string, CloudFile | undefined>;
};
...
export async function GET(req: NextRequest) {
  try {
    ...
    const versions = (cloud.versions || [])
      .filter((version) => version.songId === songId)
    ...
    const audioResponse = await fetch(chosen.url, {
      method: 'GET',
    ...
    if (!audioResponse.ok && audioResponse.status !== 206) {
      return noStore({ error: `Cloud audio could not be read (${audioResponse.status}).` }, 502);
    }

    const headers = new Headers();
    headers.set('Content-Type', audioResponse.headers.get('content-type') || chosen.type || 'audio/mpeg');
    ...
    console.error('Song playback proxy failed:', error);
    return noStore({ error: error instanceof Error ? error.message : 'Could not load song audio.' }, 500);
  }
}
