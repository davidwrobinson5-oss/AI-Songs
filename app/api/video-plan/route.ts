...
export async function POST(req: Request) {
  ...
  try {
    ...
    const visualStyle = textField(body.visualStyle, 120, 'Cinematic Realism');
    const cameraStyle = textField(body.cameraStyle, 120, 'Mixed by section');
    const concept = textField(body.concept, 2_000);
    const story = textField(body.story, 4_000);
    const performance = textField(body.performance, 3_000);
    const location = textField(body.location, 1_000);
    const wardrobe = textField(body.wardrobe, 1_000);
    const colorNotes = textField(body.colorNotes, 1_000);
    const mustHave = textField(body.mustHave, 1_000);
    const avoid = textField(body.avoid, 1_000);
    const duration = textField(body.duration, 40, '3:00');
    const referenceNames = Array.isArray(body.referenceNames)
      ? body.referenceNames.filter((value): value is string => typeof value === 'string').slice(0, 20).join(', ')
      : '';

    if (!process.env.OPENAI_API_KEY) {
      return NextResponse.json({ error: 'Video planning is temporarily unavailable.' }, { status: 503 });
    }
    ...
    const response = await client.responses.create({
      ...
      input: [
        {
          role: 'system',
          content: 'You are a world-class music video director, treatment writer, storyboard artist, editor, and AI-video prompt designer. Build an original, visually memorable music-video plan without imitating a living director or artist. Make the plan shootable and AI-render friendly. Prioritize continuity, strong hero images, emotional progression, performance coverage, visual motifs, section contrast, and practical transitions. Return a concise but complete production plan with headings.'
        },
        {
          role: 'user',
