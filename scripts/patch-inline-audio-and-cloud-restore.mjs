...
// 1) Preserve/merge restored cloud audio into IndexedDB even when the version metadata already exists.
...
if (!store.includes(newImport)) {
  if (!store.includes(oldImport)) throw new Error('Cloud library version import block not found.');
  store = store.replace(oldImport, newImport);
  fs.writeFileSync(storePath, store);
}

// 2) Restore cloud audio when metadata exists locally but one or more audio blobs are missing.
...
const newRestore = `  const refreshedLocal = await exportLocalLibrary();
  ...
  if (downloaded.length) await importCloudLibrary([], downloaded);`;
if (!sync.includes(newRestore)) {
  if (!sync.includes(oldRestore)) throw new Error('Cloud audio restore block not found.');
  sync = sync.replace(oldRestore, newRestore);
  fs.writeFileSync(syncPath, sync);
}

// 3) Make original recordings and separated stems play fully in place from their thumbnail,
// using the same Play/Stop interaction as the Songs library instead of opening a new screen/tab.
...
if (!captured.includes(`const [playingFileKey,setPlayingFileKey]`)) {
  if (!captured.includes(stateNeedle)) throw new Error('Captured file state block not found.');
  captured = captured.replace(stateNeedle, `${stateNeedle}\n  const [playingFileKey,setPlayingFileKey]=useState('');`);
}
...
const inlineRows = `  function fileVisual(key:string){
  ...
  async function toggleInlineFileAudio(key:string,url:string){
    ...
    try{
      ...
      audio.dataset.pieInlineFilePlayer=key;
      audio.src=objectUrl;
      audio.preload='auto';
      ...
      const cleanup=()=>{
        ...
        audio.remove();
        setPlayingFileKey(current=>current===key?'':current);
      };
      audio.addEventListener('ended',cleanup,{once:true});
      audio.addEventListener('error',cleanup,{once:true});
      audio.addEventListener('pause',cleanup,{once:true});
      setPlayingFileKey(key);
      setFileStatus('Playing full audio.');
      await audio.play();
    }catch(error){
      setPlayingFileKey('');
      setFileStatus(error instanceof Error?error.message:'Audio could not be played.');
    }
  }
...
captured = captured.slice(0,rowStart)+inlineRows+captured.slice(selectedStart);
