...
if(source.includes(functionAnchor)&&!source.includes('async function refreshStudioInputs()')){
  const functions=`  async function refreshStudioInputs() {
    try {
      ...
      setStudioStatus(devices.length?`${devices.length} audio input${devices.length===1?'':'s'} found. Choose your USB/interface input below.`:'No audio input was found. Check the USB-C connection and try again.');
    } catch {
...
const renderAnchor=`        {audioUrl && <audio controls src={audioUrl} />}
      </div>
...
if(source.includes(renderAnchor)&&!source.includes('Plug In & Record Live')){
  const replacement=`        {audioUrl && <audio controls src={audioUrl} />}
      </div>
