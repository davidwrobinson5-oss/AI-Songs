...
// Keep the approved pie + headphones icon at the original wider proportions.
// The first horizontal asset was too narrow, which made the mark look smashed on phones.
const horizontalLogoPath = 'public/pieinears-horizontal.svg';
if (fs.existsSync(horizontalLogoPath)) {
  let logo = fs.readFileSync(horizontalLogoPath, 'utf8');
  logo = logo.replace(
    'transform="translate(10 10)" fill="none"',
    'transform="translate(10 10) translate(70 0) scale(1.36 1) translate(-70 0)" fill="none"'
  );
  fs.writeFileSync(horizontalLogoPath, logo);
}
...
console.log('Applied approved horizontal Pieinears brand layer with corrected icon proportions.');
