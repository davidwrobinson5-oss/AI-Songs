...
const phoneHash = (value) => createHash('sha256').update(String(value || '').trim()).digest('hex');
...
const matchesTarget = (user) =>
  (user.email_addresses || []).some((entry) => hash(entry.email_address) === expectedEmailHash) ||
  (user.phone_numbers || []).some((entry) => phoneHash(entry.phone_number) === expectedPhoneHash);
